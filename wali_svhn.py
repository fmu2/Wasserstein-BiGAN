...
def main():
  ...
  svhn = datasets.SVHN('~/torch/data/SVHN', split='extra', transform=transform)
  ...
  while curr_iter < ITER:
    for batch_idx, (x, _) in enumerate(loader, 1):
      ...
      if curr_iter % 100 == 0:
        ...
        utils.save_image(rect_imgs * 0.5 + 0.5, 'svhn/rect%d.png' % curr_iter)
        utils.save_image(genr_imgs * 0.5 + 0.5, 'svhn/genr%d.png' % curr_iter)
        wali.train()

      # save model
      if curr_iter % (ITER // 10) == 0:
        torch.save(wali.state_dict(), 'svhn/models/%d.ckpt' % curr_iter)
  ...
  plt.ylabel('Loss')
  plt.legend()
  plt.savefig('svhn/loss_curve.png')
