...
#     Conv2d(NUM_CHANNELS, 64, 2, 1, bias=False), BatchNorm2d(64), LeakyReLU(LEAK, inplace=True),
#     Conv2d(64, 128, 7, 2, bias=False), BatchNorm2d(128), LeakyReLU(LEAK, inplace=True),
#     Conv2d(128, 256, 5, 2, bias=False), BatchNorm2d(256), LeakyReLU(LEAK, inplace=True),
#     Conv2d(256, 256, 7, 2, bias=False), BatchNorm2d(256), LeakyReLU(LEAK, inplace=True),
...
#   mapping = nn.Sequential(
#     ConvTranspose2d(NLAT, 512, 4, 1, bias=False), BatchNorm2d(512), LeakyReLU(LEAK, inplace=True),
#     ConvTranspose2d(512, 256, 7, 2, bias=False), BatchNorm2d(256), LeakyReLU(LEAK, inplace=True),
#     ConvTranspose2d(256, 256, 5, 2, bias=False), BatchNorm2d(256), LeakyReLU(LEAK, inplace=True),
#     ConvTranspose2d(256, 128, 7, 2, bias=False), BatchNorm2d(128), LeakyReLU(LEAK, inplace=True),
#     ConvTranspose2d(128, 64, 2, 1, bias=False), BatchNorm2d(64), LeakyReLU(LEAK, inplace=True),
#     Conv2d(64, NUM_CHANNELS, 1, 1, bias=False), Tanh())
#   return DeterministicConditional(mapping)
...
#     Conv2d(NUM_CHANNELS, 64, 2, 1), LeakyReLU(LEAK),
#     Conv2d(64, 128, 7, 2), LeakyReLU(LEAK),
#     Conv2d(128, 256, 5, 2), LeakyReLU(LEAK),
#     Conv2d(256, 256, 7, 2), LeakyReLU(LEAK),
#     Conv2d(256, 512, 4, 1), LeakyReLU(LEAK))

#   z_mapping = nn.Sequential(
#     Conv2d(NLAT, 1024, 1, 1), LeakyReLU(LEAK),
#     Conv2d(1024, 1024, 1, 1), LeakyReLU(LEAK))

#   joint_mapping = nn.Sequential(
#     Conv2d(1536, 2048, 1, 1), LeakyReLU(LEAK),
#     Conv2d(2048, 2048, 1, 1), LeakyReLU(LEAK),
#     Conv2d(2048, 1, 1, 1))
...
def main():
  ...
  celeba = datasets.ImageFolder('~/torch/data/CelebA-Aligned', transform=transform)
  loader = data.DataLoader(celeba, BATCH_SIZE, shuffle=True, num_workers=2)
  ...
  while curr_iter < ITER:
    for batch_idx, (x, _) in enumerate(loader, 1):
      ...
      if curr_iter % 100 == 0:
        ...
        utils.save_image(rect_imgs * 0.5 + 0.5, 'celeba/rect%d.png' % curr_iter)
        utils.save_image(genr_imgs * 0.5 + 0.5, 'celeba/genr%d.png' % curr_iter)
        wali.train()

      # save model
      if curr_iter % (ITER // 10) == 0:
        torch.save(wali.state_dict(), 'celeba/models/%d.ckpt' % curr_iter)
  ...
  plt.ylabel('Loss')
  plt.legend()
  plt.savefig('celeba/loss_curve.png')
